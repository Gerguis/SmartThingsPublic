...
def reportStatus() {
	login()
    log.info "Status of cameras in ${location}:${getStatus(camera1)}${getStatus(camera2)}${getStatus(camera3)}"
    logout()
}
...
def activateCameras() {
    ...
    if (location.mode != sleepAlarmMode) {	//enable motion alerts only when not in sleepAlarmMode
...
private pauseRecording(cameraX) {
	if (state.session && cameraX) {
		httpPostJson("${serverURL}/json", [cmd: "camconfig", camera: camera1, pause: -1, session: state.session]) {
        	response ->
            	log.debug "pauseRecording: $response.data"
        }
    }
}
...
private unpauseRecording(cameraX) {
	if (state.session && cameraX) {
		httpPostJson("${serverURL}/json", [cmd: "camconfig", camera: camera1, pause: 0, session: state.session]) {
        	response ->
            	log.debug "unpauseRecording: $response.data"
        }
    }
}
...
private enableMotion(cameraX) {
	if (state.session && cameraX) {
		httpPostJson("${serverURL}/json", [cmd: "camconfig", camera: camera1, motion: true, session: state.session]) {
        	response ->
            	log.debug "enableMotion: $response.data"
        }
    }
}
...
private disableMotion(cameraX) {
	if (state.session && cameraX) {
		httpPostJson("${serverURL}/json", [cmd: "camconfig", camera: camera1, motion: false, session: state.session]) {
        	response ->
            	log.debug "disableMotion: $response.data"
        }
    }
}
...
private enableCamera(cameraX) {
	if (state.session && cameraX) {
		httpPostJson("${serverURL}/json", [cmd: "camconfig", camera: camera1, enable: true, session: state.session]) {
        	response ->
            	log.debug "enableCamera: $response.data"
        }
    }
}
...
private disableCamera(cameraX) {
	if (state.session && cameraX) {
		httpPostJson("${serverURL}/json", [cmd: "camconfig", camera: camera1, enable: false, session: state.session]) {
        	response ->
            	log.debug "disableCamera: $response.data"
        }
    }
}
...
private getStatusDetails(cameraX) {
	if (state.session && cameraX) {
		httpPostJson("${serverURL}/json", [cmd: "camconfig", camera: camera1, session: state.session]) {
        	response ->
            	log.debug "getStatusDetails: $response.data"
